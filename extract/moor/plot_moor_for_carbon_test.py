...
df['TIC_alt'] = bgc.create_bio_var(df['salt'].to_numpy(),'TIC')
df['alkalinity_alt'] = bgc.create_bio_var(df['salt'].to_numpy(),'alkalinity')
df['oxygen_alt'] = bgc.create_bio_var(df['salt'].to_numpy(),'oxygen')
df['NO3_alt'] = bgc.create_bio_var(df['salt'].to_numpy(),'NO3')
...
df.plot(y=['TIC','TIC_alt'],ax=ax)

z = np.mean(ds.z_rho[:,nz].values)
ax.set_title('%s, z = %0.1f m' % (moor_fn.parent.parent.name,z))

ax = fig.add_subplot(412)
df.plot(y=['alkalinity','alkalinity_alt'],ax=ax)

ax = fig.add_subplot(413)
df.plot(y=['oxygen','oxygen_alt'],ax=ax)

ax = fig.add_subplot(414)
df.plot(y=['NO3','NO3_alt'],ax=ax)
