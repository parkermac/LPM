"""
Module of functions for the Fennel et al. (2006) NPZD model.

These generally expect and return 1-D vectors because they are
to be used in a 1-D (time, z) toy model.
"""
...
theta_max = 0.053 # max ration of chlorophyll to phytoplankton biomass [mg Chl (mg C)-1] ~ [dimensionless]
...
# w_S = 5 # small detritus sinking [m d-1]
# w_L = 80 # large detritus sinking [m d-1]
...
k_P = 2 # half-saturation conc. of phytoplankton ingestion [(mmol N m-3)^2]
...
def get_I(I_0, z_rho, z_w, Chl, par=0.43, K_w=0.04, K_chl=0.025):
    ...
    K_chl = light attenuation coeff. for chlorophyll [(mg Chl)-1 m2] (corrected)
...
def get_g(Phy, g_max=0.6, k_P=2):
    """
    Hollings-type s-shaped grazing curve.
    
    Input:
    Phy = phytoplankton [mmol N m-3]
    g_max = max grazing rate [(mmol N m-3)-1 d-1]
    k_P = half-saturation conc. for phytoplankton ingestion [(mmol N m-3)^2]
...
def get_n(I, n_max=0.05, Io=0.0095, k_I=0.1):
    """
    ...
    I = photosynthetically available radiation [W m-2]
    n_max = max rate [d-1]
    k_I = light intensity at which inhibition of nitrification is half-saturated [W m-2]
    Io = threshold for light inhibition of nitrification [W m-2]
