...
This code directly compares 2 or more versions.
"""
...
H = 100 # max depth [m]
N = 100 # number of vertical grid cells
Dz = H/N
...
# plotting
plt.close('all')
pfun.start_plot(fs=12)

fig = plt.figure(figsize=(16,12))
ax1 = fig.add_subplot(211)
ax2 = fig.add_subplot(212)

for banas in [False, True]:
    ...
    if banas == False:
        linestyle = '-'
        ax1.set_title('Solid = Fennel, Dashed = Banas')
    elif banas == True:
        linestyle = '--'
    
    # colors
    c_dict = {'Phy':'g', 'Zoo':'violet', 'SDet':'tan', 'LDet':'brown',
            'NO3':'blue', 'NH4':'cyan', 'Lost':'gray'}
        
    for vn in vnr_list:
        if vn == 'NO3':
            pass
        else:
            ax1.plot(TRvec, R[vn], label=vn, lw=2, ls=linestyle, c=c_dict[vn])
    if banas == False:
        ax1.legend()
    ax1.grid(True)
    ax1.set_ylabel('Net N [mmol N m-2]')
    
    for vn in ['NO3']:
        ax2.plot(TRvec, R[vn], label=vn, lw=2, ls=linestyle, c=c_dict[vn])
    if banas == False:
        ax2.legend()
    ax2.grid(True)
    ax2.set_xlabel('Days')
    ax2.set_ylabel('Net N [mmol N m-2]')
