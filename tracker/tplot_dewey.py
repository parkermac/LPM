...
Like tplot.py but it only plots the map panel, and defaults to the
full domain.
"""
...
# also mask out times after a specified time
dt_last = datetime(2023,9,29,17,0,0)
t_mask = np.zeros(len(dt_list), dtype=bool)
ii = 0
for dt in dt_list:
    if dt <= dt_last:
        t_mask[ii] = True
...
lon = dsr.lon.values[t_mask,::step]
lat = dsr.lat.values[t_mask,::step]
