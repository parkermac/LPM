...
# legend size
leg_dict = {'NO3 (uM)':25, 'DIN (uM)':25, 'NH4 (uM)':2, 'DO (uM)': 200}
...
for gtx in gtx_list:
    ...
    if gtx == 'cas6_v0_live':
        mod_df['DIN (uM)'] = mod_df['NO3 (uM)']
    else:
        mod_df['DIN (uM)'] = mod_df['NO3 (uM)'] + mod_df['NH4 (uM)']
        ...
        for sn in sta_list_mb + sta_list_hc + sta_list_w:
            ...
            for jj in range(len(mo)):
                ...
                if False:
                    ...
                    fac_dict = {'NH4 (uM)':1, 'NO3 (uM)':0.6, 'DIN (uM)':0.6, 'DO (uM)':0.05}
                    if MO >=0:
                        c = 'r'
                    else:
                        c = 'b'
                    ...
                    fac_dict = {'NH4 (uM)':3, 'NO3 (uM)':0.4, 'DIN (uM)':0.4, 'DO (uM)':0.03}
                    ...
                    if (sn == sta_list_hc[0]) and (cc==0) and do_legend:
                        ax.plot(ii+.5,-200, marker='o', ls='',
                            ms=fac_dict[vn]*leg_dict[vn],
                            c='r', alpha=.3)
                        ax.text(ii+1,-200,'Obs %s = %d' % (vn, leg_dict[vn]), c = 'r',
                            fontweight='bold', va='center', alpha=.5)
                        ax.plot(ii+.5,-230, marker='o', ls='',
                            ms=fac_dict[vn]*leg_dict[vn],
                            mec='k', mfc='None')
                        ax.text(ii+1,-230,'Mod %s = %d' % (vn, leg_dict[vn]), c = 'k',
