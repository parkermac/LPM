...
def update_v(v, E, dt):
    ...
    v['oxy'] = v['oxy'] + (138/16) * cff3 * v['NO3'] + (106/16) * cff4 * v['NH4']
    ...
    v['oxy'] = v['oxy'] - (106/16) * (cffS * v['SDet'] + cffL * v['LDet'])
    ...
    v['oxy'] = v['oxy'] - 2 * (cff * v['NH4'])
        
    return v

def airsea_oxygen(Oxy_surf, dtdays, DA, Uwind=5, Vwind=5, temp_surf=10, salt_surf=25):
    """
    Adapted from:
    ...
    UNITS NOTES:
    dtdays [day]
    0.31 [(cm/h) (m/s)-2] units from Wanninkhof (2014)
    24 [h/day]
    100 [cm/m]

    Hence the units of cff2_air are: [s2 m-1]
    """
    ...
    Oxy_air_flux_sum = cff3 * (O2satu - Oxy_surf) * DA
    """
    Units:
    cff3 [m]
    O2satu - Oxy_surf [mmol m-3]
    DA [m2]
    Oxy_air_flux_sum [mmol] = added or subtracted mmol over the
        time period dt_days

    """

    return Oxy_air_flux_sum
